...
# wire  consists of wire sections c(x1, y1, x2, y2)

process_instruction<- function(x1, y1, inst){
  dir = str_sub(inst, 1, 1)
  len = as.integer(str_sub(inst, 2,-1))
  x2=x1
  y2=y1
  if (dir=="R") x2 =x1+len
  else if (dir=="L") x2= x1-len
  else if (dir=="U") y2= y1+len
  else if (dir=="D") y2= y1-len
  c(x1,y1,x2,y2) # returns a path segment
}


wire_from_path <- function(path){
  x1 = 0
  y1 = 0
  wire = list()
  for (p in path) {
    wn = process_instruction(x1,y1, p)
    print(wn)
    x1=wn[3]
    y1=wn[4]
    wire = append(wire,list(wn))
  }
  wire
}

is_between <- function(t, a, b ) return ((a-t)*(b-t) < 0)


find_intersection <- function(w1, w2){
  
}

  
check_for_intersection <- function(w1, w2){
  # check whether w1 and w2 intersect
  # wires intersect, if
  # w21 is between w11 and w13
  if ( (is_between(w1[2], w2[2], w2[4])) && (is_between(w2[1],w1[1], w1[3]))) return(c(w2[1], w1[2]))
  if ( (is_between(w2[2], w1[2], w1[4])) && (is_between(w1[1],w2[1], w2[3]))) return(c(w1[1], w2[2]))
  return (FALSE)
}


intersections_of_paths <- function(w1,w2){
  intersections = list()
  for (ws1 in w1){
    for (ws2 in w2){
...
manhattan_dist <- function(vec) {
  abs(vec[1])+abs(vec[2])
  }

find_minimal_manhattan <- function(w1, w2){
  intersections = intersections_of_paths(w1,w2)
  
  manhattan_distances = map_dbl(intersections, manhattan_dist)
